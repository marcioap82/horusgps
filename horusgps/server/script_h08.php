...
function abrirArquivoLog($imeiLog) {
	...
	$fn = "./var/www/html/server/logs/Log_". trim($imeiLog) .".log";
	$fn = trim($fn);
	 ...
	 // }
	  ...
	  # If we know the imei of the phone and there is a pending command send it.
	    ...
	    if ($conn_imei != "") {
			...
			$sendcmd = '*HQ,'.$conn_imei.',CR#';
			socket_send($socket, $sendcmd, strlen($sendcmd), 0);
			//echo 'ENVIOU ---------------------------->'.$send_cmd;			
			...
			if ($tipoComando == "arquivo" and file_exists("$command_path/$conn_imei")) {
				...
				if ($tipoComando == "banco") {
					...
					$cnx = mysqli_connect("localhost", "root", "horus4321") or die("Could not connect: " . mysql_error());
					...
					if (mysqli_num_rows($res) >0){
						while($data = mysqli_fetch_assoc($res)) {
							...
							$sendcmd = $data['command'];
						}
						// Deletando comando
						mysqli_query($cnx,"DELETE FROM command WHERE imei = $conn_imei");
						printLog($fh, "Comando apagado do banco: " . $sendcmd . " imei: " . $conn_imei);
						
						socket_send($socket, $sendcmd, strlen($sendcmd), 0);
						printLog($fh, "Comando enviado: " . $sendcmd . " imei: " . $conn_imei);
					}
					mysqli_close($cnx);
		...
		if ($rec != "") {
			...
			if(strtoupper(substr($arCommands[0],1,2)) == 'HQ'){				
				$isH08 = true;
			}
			/*
			0 = *HQ,
			...
			*/
			...
			if ($isH08 && count($arCommands) > 7 ){
				...
				if ($arCommands[3]=="B"){
					...
					$latitudeDecimalDegrees = $arCommands[4];
					$latitudeHemisphere = $arCommands[5];
					$longitudeDecimalDegrees = $arCommands[6];
					$longitudeHemisphere = $arCommands[7];
					$speed = $arCommands[8];
					...
					/*************************** <CERCA> *****************************/
					$conx = mysql_connect("localhost", "root", "horus4321") or die("Could not connect: " . mysql_error());
					mysql_select_db('tracker',$conx);
					$consulta = mysql_query("SELECT * FROM geo_fence WHERE imei = '$imei'", $conx);
					...
					while($data = mysql_fetch_assoc($consulta)){		
						...
						if(round($speed * 1.852, 0) > 0){
							$getCondicaoCerca = mysql_query("SELECT mensagem FROM alertas WHERE imei = '$imei' AND mensagem LIKE '%violada%' ORDER BY data DESC",$conx);
	
							$dados = mysql_fetch_assoc($getCondicaoCerca);
							$condicaoCerca = explode(" - ", $dados['mensagem']);
	
							if(@$condicaoCerca[1] != $situacao){
								mysql_query("INSERT INTO alertas (imei, mensagem, data) VALUES ('$imei', 'Cerca $nomeCerca violada - $situacao', UNIX_TIMESTAMP())",$conx);
							}
							...
							if($tipoEnvio == 0){
								...
								$consulta1 = mysql_query("SELECT a.*, b.* FROM cliente a INNER JOIN bem b ON a.id = b.cliente WHERE b.imei = '$imei'",$conx);
								
								while($data = mysql_fetch_assoc($consulta1)) {	
									//$emailDestino = $data['email'];
									$emailDestino = 'otagomes@hotmail.com';
									$nameBem = $data['name'];
									...
									$mensagem->Username = "otavio.brandon@gmail.com";
									$mensagem->Password = "Natan123";
									$mensagem->From = "otavio.brandon@gmail.com";
				...
				if ($arCommands[3]=="A"){
					...
					/*************************** <CERCA> *****************************/
					$consulta = mysqli_query($con, "SELECT * FROM geo_fence WHERE imei = '$imei'");
					...
					while($data = mysqli_fetch_assoc($consulta)){		
						...
						if(round($speed * 1.852, 0) > 0){
							$getCondicaoCerca = mysqli_query($con, "SELECT mensagem FROM alertas WHERE imei = '$imei' AND mensagem LIKE '%violada%' ORDER BY data DESC");
	
							$dados = mysqli_fetch_assoc($getCondicaoCerca);
							$condicaoCerca = explode(" - ", $dados['mensagem']);
	
							if(@$condicaoCerca[1] != $situacao){
								mysqli_query($con, "INSERT INTO alertas (imei, mensagem, data) VALUES ('$imei', 'Cerca $nomeCerca violada - $situacao', UNIX_TIMESTAMP())");
							}
							...
							if($tipoEnvio == 0){
								...
								$consulta1 = mysqli_query($con, "SELECT a.*, b.* FROM cliente a INNER JOIN bem b ON a.id = b.cliente WHERE b.imei = '$imei'");
			...
			elseif ($arCommands[2]=="XT"){
				//heartbeat
				echo $rec.'------------------- HEARTBEAT (XT) -------------';
				...
				// replicar ultima posição - posi o xt só informa o heartbeat - significa que o carro está parado
				$cnx = mysql_connect("localhost", "root", "horus4321") or die("Could not connect: " . mysql_error());
				mysql_select_db('tracker', $cnx);
				$res = mysql_query("insert into gprmc
									select null as id,
										   now() as date,
										   imei,
										   null,
										   ...
										   voltagem_bateria,
										   carregamento
									 from gprmc where date = (select max(date)
																from gprmc
															   where imei = '".$conn_imei."')", $cnx);
															   
				$conx = mysqli_connect("localhost", "root", "horus4321") or die("Could not connect: " . mysqli_error());
				mysqli_select_db($conx,'tracker');
				mysqli_query($conx,"UPDATE bem set date = now(), status_sinal = 'R' WHERE imei = '$imei'");   
				mysqli_query($conx,"UPDATE loc_atual set date = now(), status_sinal= 'R' where imei = '$imei'");
				mysqli_close($conx);
			}
		}
		$rec = "";
			...
			if($resCliente !== false){
				...
				if ($gpsSignalIndicator != 'L') {
					...
					*/
					...
					if($numRows == 0){
						mysql_query("INSERT INTO loc_atual (date, imei, phone, satelliteFixStatus, latitudeDecimalDegrees, latitudeHemisphere, longitudeDecimalDegrees, longitudeHemisphere, speed, infotext, gpsSignalIndicator, converte, ligado, status_sinal, address) VALUES (now(), '$imei', '$phone', '$satelliteFixStatus', '$latitudeDecimalDegrees', '$latitudeHemisphere', '$longitudeDecimalDegrees', '$longitudeHemisphere', '$speed', '$infotext', '$gpsSignalIndicator', 1, '$ligado', 'R', '$address')", $con);
					} else {
						mysql_query("UPDATE loc_atual set date = now(), phone = '$phone', satelliteFixStatus = '$satelliteFixStatus', latitudeDecimalDegrees = '$latitudeDecimalDegrees', latitudeHemisphere = '$latitudeHemisphere', longitudeDecimalDegrees = '$longitudeDecimalDegrees', longitudeHemisphere = '$longitudeHemisphere', speed = '$speed', infotext = '$infotext', gpsSignalIndicator = '$gpsSignalIndicator', converte = 1, ligado = '$ligado', status_sinal = 'R', address = $address where imei = '$imei'", $con);
					}
					...
					if($numRows == 0){
					mysql_query("INSERT INTO loc_atual (date, imei, phone, satelliteFixStatus, latitudeDecimalDegrees, latitudeHemisphere, longitudeDecimalDegrees, longitudeHemisphere, speed, infotext, gpsSignalIndicator, address, converte) VALUES (now(), '$imei', '$phone', '$satelliteFixStatus', '$latitudeDecimalDegrees', '$latitudeHemisphere', '$longitudeDecimalDegrees', '$longitudeHemisphere', '$speed', '$infotext', '$gpsSignalIndicator' ,'$address',1)", $con);
					} else {
					mysql_query("UPDATE loc_atual set date = now(), phone = '$phone', satelliteFixStatus = '$satelliteFixStatus', latitudeDecimalDegrees = '$latitudeDecimalDegrees', latitudeHemisphere = '$latitudeHemisphere', longitudeDecimalDegrees = '$longitudeDecimalDegrees', longitudeHemisphere = '$longitudeHemisphere', speed = '$speed', infotext = '$infotext', gpsSignalIndicator = '$gpsSignalIndicator', address = '$address', converte = 1 where imei = '$imei'", $con);
					}
