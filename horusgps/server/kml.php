...
$loop = 0;
$res1 = mysql_query("SELECT b.id as ide, b.name as nomeBem, b.identificacao FROM bem b WHERE b.imei = $imei LIMIT 1");
...
$res = mysql_query("SELECT id, infotext, date, latitudeDecimalDegrees, longitudeDecimalDegrees, latitudeHemisphere, longitudeHemisphere, speed, address FROM gprmc WHERE gpsSignalIndicator = 'F' and imei = '$imei' ORDER BY date DESC, id DESC LIMIT 15");
$line_coordinates = "";
$ballons = "";
...
while($data = mysql_fetch_assoc($res)) {
	...
	if ($loop != 0) {
		...
		<Placemark>
			...
			<description>Velocidade : '.floor($speed).'Km/h - Data : '.date('d/m/Y H:i:s', strtotime($datat)).' &lt;br/&gt; 
...
$res = mysql_query("SELECT id, extended_info, time, latitude, longitude, speed, address FROM positions WHERE device_id = '". $idoutros. "' and valid = 1 ORDER BY time DESC, id DESC LIMIT 15");
$line_coordinates = "";
$ballons = "";
...
while($data = mysql_fetch_assoc($res)) {
		...
		$trackerdate = ereg_replace("^(..)(..)(..)(..)(..)$","\\3/\\2/\\1 \\4:\\5",$data['time']);
		$datat = $data['time'];
    ...
    <Style id="highlightPlacemark">
      <IconStyle>
        <Icon>
         ...
         <!-- <href>http://www.locatemycar.com.br/imagens/GPSCar.png</href> -->
        </Icon>
      </IconStyle>
    </Style>
