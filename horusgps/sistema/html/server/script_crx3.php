...
function interact($socket) {
	...
	while (@socket_recv($socket, $rec, 2048, 0x40) !== 0) {
		if (!empty($conn_imei)) {
			$cnx = mysqli_connect("localhost", "root", "horus4321", "tracker") or die("Não foi possivel conectar ao Mysql".mysqli_error());
			/* COMANDOS*/
			...
			if ($isCRX3) {
				...
				for ($i = 0; $i < count($tmpArCommand); $i++) {
					...
					if ($protocolNumber == '01') {
						...
						$con = new mysqli("localhost", "root", "horus4321", "tracker");
						if ($con) {
...
function enviaEmail($destinatario, $assunto, $msgConteudo) {
	...
	$mail->SMTPSecure  = 'tls';
	$mail->SMTPAuth    = true;
	$mail->Username    = "pjprogramacao@gmail.com";
	$mail->Password    = "jorge1980";
	$mail->setFrom('pjprogramacao@gmail.com', 'CRLinux Soluções de TI');
	$mail->addReplyTo('pjprogramacao@gmail.com', 'CRLinux Soluções de TI');
...
function atualizarBemSerial($imei, $serial) {
	$con = mysqli_connect("localhost", "root", "horus4321", "tracker") or die("Não foi possivel conectar ao Mysql".mysqli_error());
	if ($con !== false) {
