...
<body>
	<head>
		...
		<link rel="stylesheet" href="./css/swall.css">
		...
		<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDjMfGUKYd1fIrIN5LXwC_8FofXuCOkxWU" type="text/javascript"></script>
	...
	<body>
		<section id="wrapper">
			<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
				<section class="navbar-header" style="height: 54px;">
					...
					<a href="JavaScript:location.reload(true);"><section  id="logoEmpresa"></section></a>
				</section>
					...
					<ul class="nav navbar-nav navbar-right navbar-user">
						...
						<li class="dropdown veiculos-dropdown">
							...
							<ul class="dropdown-menu text-center">
								...
								<li class="divider"></li>
								<li id="viewrotadel" class="disabled "><a href="#" onclick="delLine()" data-toggle="modal">
								<i class="fa icon-map-marker"></i> Remover Trajeto</a></li>
								<li id="viewrotaadd" class="disabled "><a href="#" onclick="addLine()" data-toggle="modal">
								<i class="fa icon-map-marker"></i> Adcionar Trajeto</a></li>
							</ul>
						</li>
						...
						<li><a href="#modal-alertas" data-toggle="modal"><i class="fa fa-bell"></i> Alertas <span  class="badge">0</span></a></a></li>
						<li class="divider-vertical"></li>
						<!-- Gerenciar -->
						...
						<li><a id="btnFinanceiro" href="#"><i class="fa fa-fw fa-usd"></i> Faturas</a></li>
	  ...
	  <!-- Imprimir Modal Histórico -->
            <section class="modal fade" id="modal-imprimir-historico">
				<section class="modal-dialog-historico">
                	<section class="modal-header"> 
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>                        
                    </section>
					<section class="modal-content"> 
						<section class="modal-body"> 
                        	<iframe id="ifrRelatorio" width="100%" height="800px" frameborder="0" scrolling="yes"></iframe>
                        </section>						
					</section>
				</section>
			</section>
            ...
            <!-- Modal Financeiro -->
            <div id="modalFinanceiro" class="modal" role="dialog">
                <div class="modal-dialog modal-lg">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <iframe height="600px" width="100%" scrolling="auto" frameborder="0" src="http://smartprotecao.com.br/financeiro/?ng=client/login/"></iframe>
                        </div>
                        <div class="modal-footer">Voltar</button>
                        </div>
                    </div>
	    	...
	    	<div id="status-sinal"  class="alert alert-info col-md-1 text-center" role="alert"><strong><i class="fa fa-signal" aria-hidden="true"></i> Status.: <br><li class="status-sinal" rel="tooltip"></li></strong></div>   
			
			<div id="widget-Ignicao" class="alert alert-info col-md-1 text-center" role="alert"><strong><i class="fa fa-key"></i> Ligado: <br><?=$data['ligado'] =='S' || $data['speed'] > 0 ? 'Sim' : 'Não'?></strong></div>
		...
		<script type="text/javascript">
			...
			function modalCerca(){
				...
				if(imei){
					...
					var riogn = new google.maps.LatLng(-5.746327, -36.469288);
					var opcoesCerca = {
						zoom: 12,
						center: riogn,
	 ...
	 mapTypeControl: true,
      ...
      streetViewControlOptions: {
        position: google.maps.ControlPosition.LEFT_TOP
      	...
      	place: {
      		...
      		location: {lat: -14.198263, lng: -56.638331}
      	},
      	attribution: {
			...
			function addPolyline(lt, lg) {
        var myLatlng = new google.maps.LatLng(lt, lg);
        var path = poly.getPath();

        path.push(myLatlng);
        var imagem = './imagens/marker_start.png';
      ...
      function delLine() {
        $('#viewrotaadd').removeClass('disabled');
        $('#viewrotadel').addClass('disabled');
        poly.setMap(null);
        // poly = [];
      }

      function addLine() {
        $('#viewrotadel').removeClass('disabled');
        $('#viewrotaadd').addClass('disabled');
      ...
      google.maps.event.addListener(thePanorama, 'visible_changed', function () {
	    ...
	    // Setar o Marker no centro do mapa
        postMap = 1;
        centrarMapa = 1;
    function centralizar(lt,lg){
    	postMap++;
    	if (postMap > 6 && centrarMapa == 1 ) {
    	var centro = new google.maps.LatLng(lt, lg);
    	var_map.setCenter(centro);
    	postMap = 1;
    	};
			}

	/* DEFININDO TODOS MARCADORES */
						...
						success: function(aDados){ 
						 ...
						 $('#viewrotadel').addClass('disabled');
							...
							for(var i = 0; i < aDados.length; i++){
							    ...
							    $("#widget-infoVeiculo").fadeIn(3000);
								$('#widget-infoVeiculo').removeClass('hide').addClass('animated fadeInUp');
								...
								google.maps.event.addListener(pointMarker, 'click', (function(pointMarker, i){
									return function(){
										infowindow.setContent(
											...
											"<br /><b>Endereço: </b>" +aDados[i]["endereco"] + 
											"</p></section>"
										);
							...
							if(aDados.length > 1){
							$('#widget-Veloc').animate({"left" :-250, "opacity" : 0}, 'fast');
							$('#widget-Ignicao').animate({"left" :-250, "opacity" : 0}, 'slow');
							$('#status-sinal').animate({"left" :-250, "opacity" : 0}, 'slow');	
								var_map.setZoom(10);
								poly.setMap(null);
							 $('#widget-Veloc').animate({"left" : 9, "opacity" : 0.9}, 'fast');
  					         $('#widget-Ignicao').animate({"left" : 9, "opacity" : 0.9}, 'slow');
					         $('#status-sinal').animate({"left" : 9, "opacity" : 0.9}, 'slow');
					...
					for (var i = 0; i < markerArray.length; i++){
						...
						$.ajax({
							...
							success: function (coordenadas){
								...
								var escreveSN;
                            escreveSN = dados.ligado =='S' || dados.velocidade > 0 ? 'Sim' : 'Não' ;

                            $('#widget-imei').html("<strong>Imei:</strong> "+dados.imei);
                            $('#widget-nome').html("<strong>Placa:</strong> "+dados.nome);
                            $('#widget-chip').html("<strong>Chip:</strong> "+dados.chip);
                            $('#widget-modelo').html("<strong>Modelo:</strong> "+dados.modelo);
                            $('#widget-identificacao').html("<strong>N.:</strong> "+dados.identificacao);
                            $('#widget-endereco').html("<strong>Endereço:</strong> "+dados.endereco);
                            $('#widget-Veloc').html("<strong><i class='fa fa-tachometer'></i> Veloc.: <br>"+dados.velocidade+" Km/h</strong>");
                            $('#widget-Ignicao').html("<strong><i class='fa fa-key'></i> Ignição: <br>"+escreveSN+"</strong>");

					/* chama posição que centraliza o mapa */
						if (coordenadas.length == 1) {
							centralizar(dados.latitude,dados.longitude);
						};

						if (coordenadas.length == 1){
							
						$('#infoGeo').html("<strong>Imei:</strong> "+dados.imei+"&nbsp&nbsp&nbsp&nbsp<strong>N.Chip: </strong> "+dados.identificacao+"<br>"+
						"<strong>Endereço: </strong> "+dados.endereco+'&nbsp;&nbsp;<br>'+
						"<strong><i class='fa fa-tachometer'></i></strong> "+dados.velocidade+" Km/h&nbsp&nbsp&nbsp&nbsp"
							);
          ...
          if (status == google.maps.GeocoderStatus.OK) {
			...
			function drawNewAnimatedMap(){		
				...
				marker.info = new google.maps.InfoWindow({
					content: '<b>Data: </b> ' + data.horario + '<br /><b>Endereço:</b> ' + data.velocidade
				});
