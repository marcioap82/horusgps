...
<?php
	...
	function abrirArquivoLog($imeiLog){
		...
		$fn = "./var/www/server/logs/TK_" . trim($imeiLog) . ".log";
		$fn = trim($fn);
	...
	$command_path = "./var/www/server/comandos";
	...
	function interact($socket){
		$conexao = mysqli_connect("localhost", "root", "300606", "tracker");
					...
					if(strpos($parts[0], "#") === false){
						if(count($parts) > 1){
							...
							if($gpsSignalIndicator != 'L'){
								...
								*/
								$con = mysqli_connect("localhost", "rastreador", "092909", "tracker");
								mysqli_select_db($conexao, "tracker");
