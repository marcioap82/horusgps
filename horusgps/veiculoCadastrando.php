									...
									<?php
										...
										if(!verificaPost($imei, $placa, $telefone, $hodometro, $marca, $modelo, $ano, $cor) || $tipoVeiculo == "valorPadrao" || $modeloRastreador == "valorPadrao"){
											echo 
												...
												<script>veiculoCadastroIncompleto(" . $dados['id_usuario'] . ")</script>";
										}
										else{
											$getImei = mysqli_query($conexao, "SELECT * FROM bem WHERE imei = '$imei'");
											$qtdImei = mysqli_num_rows($getImei);
											
											if($qtdImei > 0){
												echo 
													"<center><img src='imagens/imgFalha.png' alt='Falha' /><br /><br />
													O número IMEI inserido já está cadastrado.<br />
													Por favor, avalie possíveis erros e tente novamente.<br /><br /></center>
													<script>veiculoCadastroIncompleto(" . $dados['id_usuario'] . ")</script>";
											}
											else{
												$getTelefone = mysqli_query($conexao, "SELECT * FROM bem WHERE identificacao = '$telefone'");
												$qtdTelefone = mysqli_num_rows($getTelefone);
												
												if($qtdTelefone > 0){
													echo 
														"<center><img src='imagens/imgFalha.png' alt='Falha' /><br /><br />
														O número de telefone inserido já está cadastrado.<br />
														Por favor, avalie possíveis erros e tente novamente.<br /><br /></center>
														<script>veiculoCadastroIncompleto(" . $dados['id_usuario'] . ")</script>";
												}
												else{
													mysqli_query($conexao, "INSERT INTO bem (imei, name, cliente, identificacao, hodometro, marca, modelo, ano, cor, tipo, modelo_rastreador, apelido) VALUES ('$imei', '$placa', $dados[id_usuario], '$telefone', '$hodometro', '$marca', '$modelo', '$ano', '$cor', '$tipoVeiculo', '$modeloRastreador', 'Desvinculado')");
													
													echo 
														"<center><img src='imagens/imgSucesso.png' alt='Sucesso' /><br /><br />
														Sucesso!<br />
														Os dados do veículo foram cadastrados.<br /><br /></center>
														<script>veiculoCadastroCompleto(" . $dados['id_usuario'] . ")</script>";
												}
											}						
										}
									?>
